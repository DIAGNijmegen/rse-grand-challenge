 ...
 *
 * In addition, the ordering is such that we can also detect tampering.
 * The later (2s) action is scheduled first, then the earlier (1s) action,
 * so that the append step is registered before the step it depends on.
 */
...
function initContactChallenge() {
    ...
    if (c2.value !== "") {
        // The challenge has already been solved (or is being solved) on this
        // form; c2 is set. Running again would schedule a duplicate round that
        // could corrupt the value (e.g. append c0 twice). Safe to no-op so
        // that calling initContactChallenge multiple times is fine.
        return;
    }
